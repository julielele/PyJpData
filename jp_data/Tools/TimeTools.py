...
def strA_2strB(dt_str, fmtA='%Y-%m-%d %H:%M:%S',fmtB='%Y%m%d%H%M%S'):
    """
    将字符串转换为其他格式的字符串
    """
    if not dt_str:
        return None
    dt = datetime.datetime.strptime(dt_str, fmtA)
    return datetime.datetime.strftime(dt, fmtB)
...
def get_time_delta(begin_dt, end_dt, time_fmt=None):
    ...
    startTime = strTimeToDatetime(begin_dt)
    endTime = strTimeToDatetime(end_dt)
...
def strTimeToDatetime(strTime):
    """
    将格式20210801080302日期转换成datetime
    """
    timeArray = time.strptime(strTime, "%Y%m%d%H%M%S")
    timeStr = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
    return string_2datetime(timeStr)

def getPosMintime(startTime,minutes):
    """
    输入datetime，返回加几分钟之后的时间字符串
    """
    #计算偏移量
    offset = datetime.timedelta(minutes=minutes)
    # 获取修改后的时间并格式化
    new_date = (startTime + offset).strftime('%Y-%m-%d %H:%M:%S')
    return new_date

def getPreMintime(startTime,minutes):
    """
    输入datetime，返回加几分钟之前的时间字符串
    """
    #计算偏移量
    offset = datetime.timedelta(minutes=minutes)
    # 获取修改后的时间并格式化
